# Sioux Falls ネットワークにおける UE・SO 配分結果の比較  
（混雑・CO₂・騒音・事故外部性）

## 1. 概要

本節では，Sioux Falls ネットワークを対象として，

- **ユーザー均衡（User Equilibrium, UE）**
- **社会的最適（System Optimum, SO）**

の配分結果を比較する。  
SO 配分では外部性を段階的に追加し，以下の 5 ケースを分析した。

### 分析ケース
- **UE**
- **SO（混雑外部性のみ）**
- **SO（混雑 + CO₂）**
- **SO（混雑 + CO₂ + 騒音）**
- **SO（混雑 + CO₂ + 騒音 + 事故）**

## 2. 指標の定義

リンク $a$ について，以下を定義する。

- $x_a$：リンク交通量 [veh]  
- $t_a(x_a)$：リンク旅行時間 [min/veh]  
- $mc_a(x_a)$：リンクの社会限界旅行時間 [min/veh]  


### (1) Total Flow

$$
\text{Total Flow} = \sum_a x_a
$$



### (2) Total Travel Time (TTT)

$$
\text{TTT} = \sum_a x_a \, t_a(x_a)
\quad [\mathrm{veh\cdot min}]
$$



### (3) Total Marginal Cost

$$
\text{Total Marginal Cost} = \sum_a x_a \, mc_a(x_a)
\quad [\mathrm{veh\cdot min}]
$$



### (4) 混雑外部性（Congestion Externality, EXT）

$$
\text{EXT} = \sum_a x_a \left( mc_a(x_a) - t_a(x_a) \right)
\quad [\mathrm{veh\cdot min}]
$$



### (5) CO₂ コスト

$$
\text{CO₂ Cost} = \sum_a x_a \, c^{CO_2}_a
\quad [\mathrm{veh\cdot min}]
$$



### (6) 騒音コスト

$$
\text{Noise Cost} = \sum_a x_a \, c^{noise}_a
\quad [\mathrm{veh\cdot min}]
$$



### (7) 事故コスト

$$
\text{Accident Cost} = \sum_a x_a \, c^{acc}_a
\quad [\mathrm{veh\cdot min}]
$$



### (8) 総社会費用（Total Social Cost, TSC）

$$
\text{TSC}=
\text{TTT}
+ \text{EXT}
+ \text{CO₂}
+ \text{Noise}
+ \text{Accident}
\quad [\mathrm{veh\cdot min}]
$$



## 3. 配分結果

|                                        | UE           | SO(congestion)   | SO(cong+CO₂)   | SO(cong+CO₂+noise)   | SO(cong+CO₂+noise+accident) |
|:---------------------------------------|:-------------|:-----------------|:---------------|:---------------------|:----------------------------|
| Total Flow                             | 8.79824×10^5 | 9.14208×10^5     | 9.13681×10^5   | 9.14063×10^5         | 9.14255×10^5                |
| Total Travel Time (TTT) [veh·min]      | 4.50155×10^6 | 4.37063×10^6     | 4.36561×10^6   | 4.36427×10^6         | 4.36831×10^6                |
| Total Marginal Cost [veh·min]          | 1.42813×10^7 | 1.32369×10^7     | 1.32204×10^7   | 1.32206×10^7         | 1.32390×10^7                |
| Congestion Externality (EXT) [veh·min] | 9.77970×10^6 | 8.86630×10^6     | 8.85479×10^6   | 8.85635×10^6         | 8.87069×10^6                |
| Total CO₂ Cost [veh·min]               | 3.90753×10^5 | 3.92379×10^5     | 3.91289×10^5   | 3.90154×10^5         | 3.90376×10^5                |
| Total Noise Cost [veh·min]             | 2.44625×10^5 | 2.46765×10^5     | 2.46344×10^5   | 2.45235×10^5         | 2.45288×10^5                |
| Total Accident Cost [veh·min]          | 5.20604×10^3 | 5.20789×10^3     | 5.20387×10^3   | 5.19569×10^3         | 5.19687×10^3                |
| Total Social Cost (TSC) [veh·min]      | 1.49218×10^7 | 1.38813×10^7     | 1.38632×10^7   | 1.38612×10^7         | 1.38799×10^7                |



## 4. 考察



### 4.3 事故コストの規模について

事故コストは，他の外部性と比べて **桁違いに小さい**。  
これは事故が低頻度事象であることを反映した **理論的に妥当な結果**である。

そのため，本研究では事故コストの基本ケースを維持し，  
**感度分析（例：×10，×50）によって影響の大きさを検証する**。



## 4.5 事故コスト感度分析（×10）

本節では，事故コストを基本ケースの **10 倍（×10）** に設定した感度分析の結果を示す。  
これは，事故リスクや社会的評価が高まった状況を想定し，配分結果および総社会費用の変化を確認することを目的としている。

### 事故コスト ×10 の配分結果

|                                        | UE           | SO(congestion)   | SO(cong+CO₂)   | SO(cong+CO₂+noise)   | SO(cong+CO₂+noise+accident) |
|:---------------------------------------|:-------------|:-----------------|:---------------|:---------------------|:----------------------------|
| Total Flow                             | 8.79824×10^5 | 9.14208×10^5     | 9.13681×10^5   | 9.14063×10^5         | 9.14255×10^5                |
| Total Travel Time (TTT) [veh·min]      | 4.50155×10^6 | 4.37063×10^6     | 4.36561×10^6   | 4.36427×10^6         | 4.36831×10^6                |
| Total Marginal Cost [veh·min]          | 1.42813×10^7 | 1.32369×10^7     | 1.32204×10^7   | 1.32206×10^7         | 1.32390×10^7                |
| Congestion Externality (EXT) [veh·min] | 9.77970×10^6 | 8.86630×10^6     | 8.85479×10^6   | 8.85635×10^6         | 8.87069×10^6                |
| Total CO₂ Cost [veh·min]               | 3.90753×10^5 | 3.92379×10^5     | 3.91289×10^5   | 3.90154×10^5         | 3.90376×10^5                |
| Total Noise Cost [veh·min]             | 2.44625×10^5 | 2.46765×10^5     | 2.46344×10^5   | 2.45235×10^5         | 2.45288×10^5                |
| Total Accident Cost [veh·min]          | 5.20604×10^4 | 5.20789×10^4     | 5.20387×10^4   | 5.19569×10^4         | 5.19687×10^4                |
| Total Social Cost (TSC) [veh·min]      | 1.49687×10^7 | 1.39281×10^7     | 1.39101×10^7   | 1.39080×10^7         | 1.39266×10^7                |



### 考察（事故コスト ×10）

事故コストを 10 倍に拡大した場合でも，以下の特徴が確認できる。

- 交通量配分および Total Travel Time は，基本ケースとほぼ同一である  
- Total Accident Cost は全ケースで比例的に増加しているが，  
  総社会費用に占める割合は依然として限定的である  
- **SO（混雑 + CO₂ + 騒音）** が引き続き最小の総社会費用を示している  

これは，事故コストを 10 倍にしてもなお，

- 混雑外部性が支配的であること  
- 事故外部性がネットワーク全体の配分を大きく変化させるには至らないこと  

を示している。

すなわち，本ネットワークにおいては，  
**安全性を強く重視した場合でも，交通配分の主要な決定要因は混雑である**  
という構造が確認された。

---

## 4.6 事故コスト感度分析（×50）

本節では，事故コストを基本ケースの **50 倍（×50）** に設定した感度分析の結果を示す。  
これは，事故リスクが極めて重視される状況を仮定し，  
安全外部性が交通配分および総社会費用に与える影響の上限を確認することを目的としている。

### 事故コスト ×50 の配分結果

|                                        | UE           | SO(congestion)   | SO(cong+CO₂)   | SO(cong+CO₂+noise)   | SO(cong+CO₂+noise+accident) |
|:---------------------------------------|:-------------|:-----------------|:---------------|:---------------------|:----------------------------|
| Total Flow                             | 8.79824×10^5 | 9.14208×10^5     | 9.13681×10^5   | 9.14063×10^5         | 9.14255×10^5                |
| Total Travel Time (TTT) [veh·min]      | 4.50155×10^6 | 4.37063×10^6     | 4.36561×10^6   | 4.36427×10^6         | 4.36831×10^6                |
| Total Marginal Cost [veh·min]          | 1.42813×10^7 | 1.32369×10^7     | 1.32204×10^7   | 1.32206×10^7         | 1.32390×10^7                |
| Congestion Externality (EXT) [veh·min] | 9.77970×10^6 | 8.86630×10^6     | 8.85479×10^6   | 8.85635×10^6         | 8.87069×10^6                |
| Total CO₂ Cost [veh·min]               | 3.90753×10^5 | 3.92379×10^5     | 3.91289×10^5   | 3.90154×10^5         | 3.90376×10^5                |
| Total Noise Cost [veh·min]             | 2.44625×10^5 | 2.46765×10^5     | 2.46344×10^5   | 2.45235×10^5         | 2.45288×10^5                |
| Total Accident Cost [veh·min]          | 2.60302×10^5 | 2.60394×10^5     | 2.60194×10^5   | 2.59785×10^5         | 2.59844×10^5                |
| Total Social Cost (TSC) [veh·min]      | 1.51769×10^7 | 1.41365×10^7     | 1.41182×10^7   | 1.41158×10^7         | 1.41345×10^7                |



### 考察（事故コスト ×50）

事故コストを基本ケースの 50 倍まで拡大した場合でも，以下の傾向が確認された。

- 交通量配分および Total Travel Time は，基本ケースおよび ×10 ケースとほぼ同一である  
- Total Accident Cost は大幅に増加しているものの，  
  **総社会費用に占める支配的要因にはなっていない**  
- **SO（混雑 + CO₂ + 騒音）** が引き続き最小の総社会費用を示している  

これは，事故コストを極端に重視した場合であっても，

- 混雑外部性が依然として最も大きな影響を持つこと  
- 事故外部性がネットワーク全体の配分を大きく変化させるには，  
  さらに強い重み付けが必要であること  

を示唆している。



### ×1・×10・×50 の比較から得られる示唆

一連の感度分析結果より，

- 事故コストを段階的に増加させても，  
  交通配分は大きく変化しない  
- Sioux Falls ネットワークでは，  
  **安全外部性よりも混雑外部性が支配的である**  

という構造が明確になった。

したがって，本研究で採用した事故コストの基本設定は過小評価ではなく，  
現実的な範囲において妥当であると結論づけられる。



